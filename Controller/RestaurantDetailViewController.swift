...
//        tableView.contentInsetAdjustmentBehavior = .never
        ...
        let heartImage = self.restaurant.isFavorite ? "heart.fill" : "heart"
    ...
    : 0.3, initialSpringVelocity: 0.7, options: [], animations: {
    ...
    @IBAction func favoriteTapped(sender: AnyObject) {
        print("This is a favorite button tapped!")
        self.restaurant.isFavorite = self.restaurant.isFavorite ? false : true
        heartButton.tintColor = self.restaurant.isFavorite ? .systemYellow : .white
        let heartImage = self.restaurant.isFavorite ? "heart.fill" : "heart"
        let heartIconConfiguration = UIImage.SymbolConfiguration(pointSize: 25, weight: .semibold)
        heartButton.image = UIImage(systemName: heartImage, withConfiguration: heartIconConfiguration)
        if let appDelegate = (UIApplication.shared.delegate as? AppDelegate) {
